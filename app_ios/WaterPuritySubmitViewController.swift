...
class WaterPuritySubmitViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func submitPurity(_ sender: AnyObject) {
        ...
        let purity = WaterPurityReport(_reporter: curAcc, _condition: condition, _viralPPM: Int(viralPPM.text!)!, _contaminantPPM: Int(contaminantPPM.text!)!, _dateTime: date.text!, place: loc, emailAddress: model.getCurrentAccount().getEmailAddress())
